...
static const int16_t kNoiseDataStds[kTableSize] = {378, 1064, 493, 582, 688, 593,
												   474, 697,  475, 688, 421, 455};
// Stds for the two Gaussians for the six channels (speech)
static const int16_t kSpeechDataStds[kTableSize] = {555, 505, 567, 524,	 585,  1231,
	...
	if (total_power > kMinEnergy) {
		...
		for (channel = 0; channel < kNumChannels; channel++) {
			...
			for (k = 0; k < kNumGaussians; k++) {
				...
				tmp1_s32 =
					WebRtcVad_GaussianProbability(features[channel], self->noise_means[gaussian],
				...
				// Value given in Q27 = Q7 * Q20.
				tmp1_s32 =
					WebRtcVad_GaussianProbability(features[channel], self->speech_means[gaussian],
		...
		for (channel = 0; channel < kNumChannels; channel++) {
			...
			noise_global_mean =
				WeightedAverage(&self->noise_means[channel], 0, &kNoiseDataWeights[channel]);
			...
			speech_global_mean =
				WeightedAverage(&self->speech_means[channel], 0, &kSpeechDataWeights[channel]);
			...
			if (diff < kMinimumDifference[channel]) {
				...
				speech_global_mean = WeightedAverage(&self->speech_means[channel], tmp1_s16,
													 &kSpeechDataWeights[channel]);

				// Move Gaussian means for noise model by -|tmp2_s16| and update
				// |noise_global_mean|. Note that |self->noise_means[channel]| is
				// changed after the call.
				noise_global_mean = WeightedAverage(&self->noise_means[channel], -tmp2_s16,
...
int WebRtcVad_CalcVad48khz(VadInstT* inst, const int16_t* speech_frame, size_t frame_length) {
	...
	for (i = 0; i < num_10ms_frames; i++) {
		WebRtcSpl_Resample48khzTo8khz(speech_frame, &speech_nb[i * kFrameLen10ms8khz],
